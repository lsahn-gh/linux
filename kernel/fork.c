...
static unsigned long *alloc_thread_stack_node(struct task_struct *tsk, int node)
{
	 ...
	 *
	 * - 위에서 vmalloc을 통해 할당이 성공했으면 해당 주소(stack)에 대한 vm area를 가져와서
	 *   tsk에 등록한다.
	 */
	if (stack) {
...
/*
 * IAMROOT, 2023.04.01:
 * - vm_area를 한개 할당해서 @orig을 복사한다.
 *   avc와 vm_next, vm_prev는 초기화한다.
 */
...
/*
 * IAMROOT, 2023.04.01:
 * - parent의 exe_file을 child에 pointer로 넣는다.
 */
...
/*
 * IAMROOT, 2023.04.01:
 * - parent mm을 child mm에 복사한다. 복사하는 과정중에 다음을 수행한다.
 *   1. exe_file 상속
 *   2. vm정보들의 상속
 *   3. parent vma를 순회하면서 복사.
 *   4. mempolicy 복사.
 *   5. page table 복사.
 *   마지막 pte는 pte의 present/ nonpresent에 따른 page 복사여부 판단, 
 *   pte mapping, 기타 처리를 수행한다.
 */
...
/*
 * IAMROOT, 2023.04.01:
 * - mm을 SINGLE_DEPTH_NESTING으로 lock한다.
 */
...
/*
 * IAMROOT, 2023.04.01:
 * - vma, mempol을 복사해오고, 복사해온 vma에 mm을 등록한다.
 */
...
/*
 * IAMROOT, 2023.04.01:
 * - VM_WIPEONFORK가 있을경우 fork시 vm을 지워야한다. 이 경우 anon_vma을 NULL로하고
 *   그게 아니면 anon_vma를 fork한다.
 *
 */
 ...
 * IAMROOT, 2023.04.01:
 * - papago
 *   VM_WIPEONFORK는 자식에서 깨끗한 슬레이트를 얻습니다.
 *   현재 vma에 대한 페이지를 복사하지 않으므로 오류가 발생할 때까지 
 *   anon_vma를 준비하지 마십시오.
 */
			tmp->anon_vma = NULL;
...
/*
 * IAMROOT, 2023.04.01:
 * - .so같은 shared file이 있는 경우로 예상할수 있다. 
 */
...
/*
 * IAMROOT, 2023.04.01:
 * - mpnt의 next에 새로생성된 vma(tmp)를 추가한다.
 */
 ...
 * IAMROOT, 2023.04.01:
 * - papago
 *   child를 위한 hugetlb 관련 페이지 예약을 지웁니다. 이는 MAP_PRIVATE 매핑에만 
 *   영향을 미칩니다. 자식이 생성한 결함은 읽기 전용인 경우에도 성공이 보장되지 
 *   않습니다. 
 */
...
/*
 * IAMROOT, 2023.04.01:
 * - 이번에 생성된 vma가 다음에는 pprev가 된다.
 *   이전의 vm_next에 vma를 등록한다.
 *   list, rb tree에 등록한다.
 *
 * - ex) pprev = &old->vm_next일 경우, 현재 생성된 tmp를 new로 가정
 *   old->vm_next = new
 *   new->vm_prev = old 
 *   prev = new
 */
...
/*
 * IAMROOT, 2023.04.01:
 * - 이전 rb_link가 새로 추가되는 tmp의 위치가 된다. 해당 위치로 leaf node개념으로
 *   tmp가 등록될것이다.
 */
		__vma_link_rb(mm, tmp, rb_link, rb_parent);
/*
 * IAMROOT, 2023.04.01:
 * - 그 다음 추가될 vma가 위치할 leaf node위치를 갱신한다.
 */
...
/*
 * IAMROOT, 2023.04.01:
 * -vma를 지우는 flag가 없엇으면 page table copy를 수행한다.
 */
		if (!(tmp->vm_flags & VM_WIPEONFORK))
...
/*
 * IAMROOT, 2023.04.01:
 * - @node에 child task를 할당하고, @orig의 값을 복사하고, 새로 생성되는거에
 *   필요한 초기화를 수행한다.
 *   새로 생성한 child task에는 stack도 새로 할당해서 넣어준다.
 */
...
/*
 * IAMROOT, 2023.04.01:
 * - node가 설정이 안되있으면 선호 node를 찾아와 본다.
 */
...
/*
 * IAMROOT, 2023.04.01:
 * - parent task(orig)를 child(tsk)에 copy한다.
 */
 ...
 * IAMROOT, 2023.04.01:
 * - papago 
 *   arch_dup_task_struct()는 스택 관련 필드를 방해합니다. 스택 관련 
 *   기능을 다시 사용하기 전에 올바르게 초기화되었는지 확인하십시오.
 *
 * - 위에서 orig를 통으로 copy했으니, 새로할당한 stack을 tsk에 넣어준다.
 */
	tsk->stack = stack;
#ifdef CONFIG_VMAP_STACK
 ...
 * IAMROOT, 2023.04.01:
 * --- Kconfig ---
 * - SECCOMP(safely execute untrusted bytecode)
 *   이 커널 기능은 실행 중에 신뢰할 수 없는 바이트코드를 처리해야 
 *   하는 숫자 계산 응용 프로그램에 유용합니다. 읽기/쓰기 시스템 
 *   호출을 지원하는 파일 디스크립터로 프로세스에 사용할 수 있는 
 *   파이프 또는 기타 전송을 사용하면 seccomp를 사용하여 자체 주소 
 *   공간에서 해당 응용 프로그램을 격리할 수 있습니다. 
 *   seccomp가 prctl(PR_SET_SECCOMP) 또는 seccomp() 시스템 호출을 
 *   통해 활성화되면 비활성화할 수 없으며 태스크는 각 seccomp 모드에서 
 *   정의된 몇 가지 안전한 시스템 호출만 실행할 수 있습니다. 
 * --------------
 *  Linux SECCOMP는 보안을 강화하기 위한 커널 기능으로, 프로세스가 
 *  시스템 콜을 호출할 때, 호출할 수 있는 콜의 종류와 매개변수를 
 *  제한하여 프로그램의 취약점을 줄입니다.
 *
 *  SECCOMP를 사용하는 대표적인 예시는 다음과 같습니다.
 *
 *  컨테이너 가상화
 *  Docker 및 Kubernetes와 같은 컨테이너 가상화 기술에서는 
 *  SECCOMP를 사용하여 컨테이너 내에서 실행되는 애플리케이션이 
 *  호스트 운영체제에 대한 접근을 제한하여 보안을 강화합니다.
 *
 *  웹 브라우저
 *  Chrome, Firefox 및 Safari와 같은 웹 브라우저는 SECCOMP를 
 *  사용하여 브라우저 내에서 실행되는 JavaScript 코드에 대한 
 *  접근을 제한합니다. 이는 브라우저의 취약점을 제한하여
 *  웹 서핑을 안전하게 만들어줍니다.
 *
 *  안드로이드 OS 
 *  Android 운영 체제는 SECCOMP를 사용하여 Android 
 *  애플리케이션의 권한을 제한하여 악성 코드의 실행을 방지합니다.
 *
 *  VPN 연결 
 *  VPN 클라이언트는 SECCOMP를 사용하여 보안을 강화하고 불필요한 
 *  프로세스를 차단하여 악성 코드의 실행을 방지합니다.
 *
 *  암호화폐 지갑
 *  암호화폐 지갑은 SECCOMP를 사용하여 사용자의 자산을 보호합니다. 
 *  지갑이 SECCOMP를 사용하지 않으면, 사용자의 자산이 해킹에 노출될 
 *  가능성이 높아집니다.
 *
 *  위와 같이, SECCOMP는 다양한 분야에서 보안을 강화하는 데 사용됩니다.
 * ---------------
 * - 참고
 ...
 * IAMROOT, 2023.04.01:
 * - papago
 *   지금과 그때 사이에 원본이 변경될 경우를 대비하여 한숨과 
 *   잠금 상태에 있으면 seccomp 필터 설정을 처리해야 합니다. 
 *   그때까지는 필터가 NULL이어야 free_task를 호출하는 오류 
 *   경로에서 사용 횟수가 엉망이 되는 것을 방지할 수 있습니다.
 *
 * - seccomp관련측도 초기화해준다.
 */
...
/*
 * IAMROOT, 2023.04.01:
 * - clear해야될 flag들을 clear하고 stack끝에 magic number를
 *   설치힌다.
 */
...
/*
 * IAMROOT, 2023.04.01:
 * - parent가 자기자신의 cpu_mask를 가리켰으면 child도 똑같이 자기자신으로
 *   설정해준다.
 */
...
/*
 * IAMROOT, 2023.04.01:
 * - rcu_users는 2부터 시작해준다(user space visible state, scheduler).
 */
...
/*
 * IAMROOT, 2023.04.01:
 * - pthtread_create, kthread인 경우 CLONE_FILES가 set되있다.
 * - share를 하는 경우(set CLONE_FILES)
 *   refup만 한다.
 * - 복사를 해야하는 경우(unset CLONE_FILES)
 *   dup한다.
 */
 ...
 */
	 ...
	 * - clone_flags에 CLONE_CHILD_SETTID가 있을경우 set_child_tid를 
	 *   args->child_tid로 설정한다.
	 * - 아래 clear_child_tid도 비슷한 논리로 설정한다.
	 */
...
/*
 * IAMROOT, 2023.04.01:
 * - UCOUNT_RLIMIT_NPROC에 대한 global max값 검사 및 해당 count부터 시작한 
 *   namespace에 설정된 local max값 over를 검사한다.
 *   즉 설정된 최대 task수를 넘었는지 검사한다.
 */
	if (is_ucounts_overlimit(task_ucounts(p), UCOUNT_RLIMIT_NPROC, rlimit(RLIMIT_NPROC))) {

/*
 * IAMROOT, 2023.04.01:
 * - ex) pid는 1, 2, 3으로 증가하는데 
 *   만약 docker가 pid는 4로 동작하는 경우 dock의 내부에선, 즉 container내부에선
 *   또 pid 1, 2, 3으로 시작하는 개념이 된다.
 */
...
/*
 * IAMROOT, 2023.04.01:
 * - parent에서 사용했던 p->mempolicy를 그대로 사용하는 상태일것이다.
 *   mempolicy를 새로 할당하면서 parent mempolicy를 복사해 새로운 mempolicy를
 *   넣는다.
 */
