...
defer_init(int nid, unsigned long pfn, unsigned long end_pfn)
{
	 ...
	 */
	/* IAMROOT, 2024.09.05:
	 * - node(@nid)의 특정 zone에서 defer_init(..)이 처음 호출되는 시점에는
	 *   prev_end_pfn != end_pfn이므로 end_pfn를 prev_end_pfn에 저장하고
	 *   nr_initialised를 0으로 초기화한다.
	 *
	 *   prev_end_pfn: previous zone의 end_pfn 값.
	 */
	...
	/* IAMROOT, 2021.12.11:
	 * - node(@nid)에 여러 zone이 존재하는 경우 상위 zone을 제외한
	 *   zone은 defer 하지 않도록 false 반환.
	 *
	 * +------+------+ --> pgdat_end_pfn (node)
	 * |      |      |
	 * |      |      |
	 ...
	 */
	/* IAMROOT, 2024.09.05:
	 * - @pfn < @end_pfn 동안 nr_initialised를 증가시킨다.
	 */
	nr_initialised++;
...
static inline bool should_skip_kasan_poison(struct page *page, fpi_t fpi_flags)
{
	return (!IS_ENABLED(CONFIG_KASAN_GENERIC) && TODO
		(fpi_flags & FPI_SKIP_KASAN_POISON)) ||
	       PageSkipKASanPoison(page);
}
...
{
	/* IAMROOT, 2024.09.06:
	 * - struct @page를 0으로 초기화한다.
	 */
	mm_zero_struct_page(page);
	/* IAMROOT, 2024.09.06:
	 * - struct @page에 @zone, @nid, @pfn 정보를 저장한다.
	 */
	set_page_links(page, zone, nid, pfn);
	/* IAMROOT, 2024.09.06:
	 * - struct @page의 refcount를 초기화한다.
	 */
	init_page_count(page);
	/* IAMROOT, 2024.09.06:
	 * - struct @page의 mapcount를 초기화한다.
	 */
		...
		struct vmem_altmap *altmap, int migratetype)
{
	 ...
	 *   application을 동작할수있는 환경이고, DRAM처럼 kernel에서 관리하기
	 *   때문에 memmap같은 자료구조가 필요하다.
	 *
	...
	for (pfn = start_pfn; pfn < end_pfn; ) {
		...
		if (context == MEMINIT_EARLY) {
			/* IAMROOT, 2021.12.11:
			 * - @zone movable 이면서 kernelcore=mirror 이면 overlap 확인하고
			 *   범위가 overlap 되면 skip 한다.
			 */
...
static void __init memmap_init(void)
{
	...
	/* IAMROOT, 2024.09.04:
	 * - memblock memory region과 node(nid)의 모든 zone을 순회하며
	 *   populated zone에 대해서만 memmap을 초기화한다.
	 */
	for_each_mem_pfn_range(i, MAX_NUMNODES, &start_pfn, &end_pfn, &nid) {
		...
		for (j = 0; j < MAX_NR_ZONES; j++) {
			...
			/* IAMROOT, 2024.09.05:
			 * - present_pages가 0인 zone은 skip한다.
			 */
...
/* IAMROOT, 2021.12.11:
 * - @zone의 per-cpu와 관련된 struct fields를 초기화한다.
 *
 *   Note.
 *   - pcp: per-cpu page
 */
static __meminit void zone_pcp_init(struct zone *zone)
{
	 ...
	 */
	/* IAMROOT, 2024.08.30:
	 * - @zone의 per_cpu_{pageset,zonestats}, pageset_{high,batch} 초기화.
	 */
					...
					unsigned long size)
{
	...
	/* IAMROOT, 2024.09.03:
	 * - zone_idx가 node의 nr zones 보다 크면 값 업데이트.
	 */
	...
	/* IAMROOT, 2024.08.30:
	 * @zone의 free_area field 초기화 및 initialized 변수 setup.
	 */
					...
					unsigned long node_end_pfn)
{
	 ...
	 * and vice versa.
	 */
	/* IAMROOT, 2024.08.23: TODO
	 * - mirrored_kernelcore bootarg가 설정된 경우이므로 현재는 skip.
	 *
	...
	/* IAMROOT, 2024.08.27:
	 * - hole을 통해 구한 absent pages 개수를 반환한다.
	 */
	return nr_absent;
}
						...
						unsigned long node_end_pfn)
{
	...
	for (i = 0; i < MAX_NR_ZONES; i++) {
		...
		/* IAMROOT, 2024.08.27:
		 * - size     : zone이 가지고 있는 모든 pages 개수.
		 *   real_size: 모든 pages 개수에서 hole pages 개수만 뺀 값.
		 *              normal/movable로 사용 가능한 pages 개수 의미.
		 */
		size = spanned;
		...
		/* IAMROOT, 2024.08.27:
		 * - struct zone 필드에 값 저장.
		 *
		 *   1) spanned_pages: hole을 포함한 node의 page 개수.
		 *   2) present_pages: hole을 제외한 node의 page 개수.
		 *   3) present_early_pages: hotplug 기능 사용시 boot 단계에서 계산한
		 *      hole을 제외한 page 개수를 tracking 하기 위해 사용.
		 */
 ...
 */
/* IAMROOT, 2024.09.03:
 * - FLATMEM 구조에서 사용되는 zone의 usemap size를 구하는 함수.
 */
static unsigned long __init usemap_size(unsigned long zone_start_pfn, unsigned long zonesize)
{
	unsigned long usemapsize;

	/* IAMROOT, 2024.09.03: TODO
	 */
...
/* IAMROOT, 2021.12.11:
 * - FLATMEM에 대한 usemap 초기화 함수.
 *   SPARSEMEM은 section 초기화하면서 usemap도 같이 초기화함.
 */
static void __ref setup_usemap(struct zone *zone)
{
	...
	if (usemapsize) {
		/* IAMROOT, 2024.09.03:
		 * - FLATMEM의 경우 pageblock_flags가 zone의 field로 되어 있다.
		 */
...
void __init set_pageblock_order(void)
{
	...
	/* IAMROOT, 2024.09.01:
	 * - 48bits, 4kb page 시스템에서는 아래와 같다.
	 *
	 *   HPAGE_SHIFT: 21
	 *   PAGE_SHIFT : 12
	 *
	 *   따라서 order는 HUGETLB_PAGE_ORDER가 되는데 계산식은 아래와 같다.
...
/* IAMROOT, 2021.12.04:
 * - @{spanned,present}_pages를 입력받아 memmap에 필요한 pages 개수를 구한다.
 */
						...
						unsigned long present_pages)
{
	 ...
	 */
	/* IAMROOT, 2021.12.04:
	 * - zone 영역의 memblock region에 hole이 있다면 hole 영역에서는
	 *   phys-memory 할당이 불가능하므로 pages 변수값을 present_pages로
	 *   보정한다.
	 *
	 *   Note.
	 *   1) 'spanned < present + @'는 hole 없거나 아주 극소량이라는 의미.
	 *   2) (present_pages >> 4)는 memmap page alignment를 위한 extra
	 *      pages 할당 의미.
	 *   3) FLATMEM은 전체 phys-memory addr-space가 연속적이므로
	 *      sparse 구조와는 다르게 hole이 발생하지 않아 zone의
	 *      spanned과 present이 동일하므로 pages 변수를 보정할 필요 없음.
	 */
	...
	/* IAMROOT, 2024.08.30:
	 * - 하나의 zone에서 PAGE_SIZE 크기의 nr @pages (frame) 들을 커버하기
	 *   위해 몇개의 page frames이 필요할지 계산하는 식.
	 *
	 *   Note. struct page를 관리하기 위해서 이를 memory에 적재할
	 *         page frames도 역시 필요하다.
	 */
...
static void __meminit pgdat_init_internals(struct pglist_data *pgdat)
{
	...
	/* IAMROOT, 2024.08.29:
	 * - split queue, kcompactd 초기화.
	 */
	pgdat_init_split_queue(pgdat);
	pgdat_init_kcompactd(pgdat);

	/* IAMROOT, 2024.08.29:
	 * - kswapd, pfmemalloc에서 사용할 waitqueue 초기화.
	 */
...
/* IAMROOT, 2021.12.11:
 * - @zone class의 member들 중에서 다음 아이템을 초기화한다.
 *   1) managed_pages
 *   2) nid
 *   3) name
 *   4) zone_pgdat
...
static void __init free_area_init_core(struct pglist_data *pgdat)
{
	...
	/* IAMROOT, 2024.08.30:
	 * - @pgdat(node)의 zone을 순회하면서 초기화한다.
	 */
	for (j = 0; j < MAX_NR_ZONES; j++) {
		...
		/* IAMROOT, 2024.08.29:
		 * - size    : hole을 포함한 zone의 page 개수.
		 *   freesize: hole을 제외한 zone의 page 개수.
		 *             buddy allocator에서 관리할 page 개수이다.
		 */
		 ...
		 */
		/* IAMROOT, 2024.08.31:
		 * - struct page 배열에 들어갈 page 개수를 구하여 memmap_pages에
		 *   저장한다.
		 */
		memmap_pages = calc_memmap_size(size, freesize);
		if (!is_highmem_idx(j)) {
			/* IAMROOT, 2021.12.04:
			 * - zone highmem이 아니면서 freesize >= memmap_pages 라면
			 *   mem_map에서 사용할 page 개수를 freesize에서 감소시킨다.
			 */
		...
		/* IAMROOT, 2021.12.04:
		 * - ZONE_DMA/DMA32 순서이면서 'freesize > dma_reserve' 조건이라면
		 *   dma_reserve에 사용할 page 개수를 freesize에서 감소시킨다.
		 *
		 *   Note. dma_reserve는 x86에서만 값이 변경되며 다른 arch는
		 *         0 이므로 freesize 값이 변동되지는 않는다.
		 */
		...
		/* IAMROOT, 2024.09.01:
		 * - 'zone != ZONE_HIGHMEM' 경우
		 *   nr_kernel_pages에 reserved pages[1]를 제외한 pages 개수를
		 *   누적시킨다.
		 *
		 * - 'zone == ZONE_HIGHMEM' 경우
		 *   nr_kernel_pages가 memmap_pages 보다 2배 이상 큰 경우
		 *   ZONE_HIGHMEM 용 memmap을 추가하기 위해 감소시킨다.
		 *
		 *   ---
		 *   [1]: memmap_pages, dma_reserve pages를 의미함.
		 */
		if (!is_highmem_idx(j))
		...
		/* IAMROOT, 2024.09.01:
		 * - reserved pages[1]를 제외한 pages 개수를 nr_all_pages에
		 *   누적시킨다.
		 *
		 *   ---
		 *   [1]: memmap_pages, dma_reserve pages를 의미함.
...
/* IAMROOT, 2021.12.04:
 * - FLATMEM 방식을 사용할 때 수행되는 node_mem_map 할당 로직.
 */
static void __init alloc_node_mem_map(struct pglist_data *pgdat)
{
	...
	/* IAMROOT, 2024.08.29:
	 * - @start : MAX_ORDER_NR_PAGES에 맞게 정렬한 뒤 설정.
	 *   @offset: 정렬하면서 생긴 pfn gap 만큼 저장.
	 */
	...
	if (!pgdat->node_mem_map) {
		...
		if (!map)
			panic("Failed to allocate %ld bytes for node %d memory map\n",
			      size, pgdat->node_id);
		/* IAMROOT, 2024.08.29: TODO
		 * - 굳이 offset을 + 하는 이유를 모르겠음.
		 *   가장 유력한 것은 buddy allocator에 맞춰 정렬하는 것임.
		 */
...
#ifdef CONFIG_DEFERRED_STRUCT_PAGE_INIT
/* IAMROOT, 2021.12.04:
 * - @pgdat의 first_deferred_pfn 값을 ULONG_MAX로 초기화한다.
 */
 ...
 */
/* IAMROOT, 2021.11.13:
 * - N_POSSIBLE이 setup된 node의 개수를 구하고 @nr_node_ids에 저장한다.
 */
 ...
 */
/* IAMROOT, 2021.11.27:
 * - node마다 zone movable 시작 pfn을 계산하기 위한 함수이다.
 *   kernelcore 영역을 계산하기 위해 zone DMA32, NORMAL 영역을 순회하며
 *   kernelcore_remaining/required_kernelcore가 0이 될때까지
 *   zone_movable_pfn 변수를 업데이트한다.
 *
 *   결국 이 함수가 종료될 떄는 zone_movable_pfn[nid] (node) 마다
 *   zone movable의 시작 pfn 값이 저장되어 있다.
 *
 *   Note: 참고 자료
 *   - memory-hotplug.rst
 */
...
restart:
	...
	/* IAMROOT, 2021.12.04:
	 * - kernelcore 영역을 [node 시작주소 .. kernelcore 크기] 만큼
	 *   하나의 node(예, node 0)에 할당하면 해당 node에 kernelcore 영역이
	 *   몰리게 된다.
	 *
	...
	for_each_node_state(nid, N_MEMORY) {
		...
		/* IAMROOT, 2021.12.04:
		 * - node (@nid)에 존재하는 memblock memory region에
		 *   kernelcore를 최대한 매핑하기 위해 compact하게 계산한다.
		 *   결국 각 node의 lowest pfn에는 kernelcore 가 저장된다.
		 *
		 *   매 loop 마다 zone_movable_pfn[nid] 값을 계속 갱신되며
		 *   이는 kernelcore_remaining가 0일때까지 계속된다.
		 *
		 *   다음 상황을 가정하고 예제 진행
		 *
		 *   1. node 마다 16GB 크기이며 ZONE_DMA32이 있다고 가정.
		 *   2. 총, DMA32: 2GB, NORMAL: 30GB
		 *   3. DMA32의 끝주소는 1GB.
		 *   4. kernelcore: 8GB
		 *
		 *   kernelcore vs. movable 비율 == 1:3 == 8:24 (GB)
		 *
		 *
		 * <---- 2번째 loop 수행
		 *   +--------+  +--------+  -+-> end_pfn
		 *   | node 1 |  | NORMAL |   |
		 *   | (16GB) |  | (15GB) |   |  --> zone movable 영역 (12GB)
		 *   |        |  |        |   |
		 *   |        |  |        |   |
		 *   |        |  |-----.  | --+-----> zone normal에서 kernelcore 영역.
		 *   |        |  | 3GB |  |
		 *   |        |  +--------+  -+-> usable_startpfn ---+
		 *   |        |  | DMA32  |   |                      | zone dma32에서
		 *   |        |  | (1GB)  |   |                      | kernelcore 영역.
		 ...
		 *
		 *
		 *   Note: 위 구조와 다르게 DMA32가 node 0에만 존재하게 되어
		 *         node 1에는 zone normal로 생성되는 경우도 있으나
		 *         계산은 위와 같음.
		 */
		...
		for_each_mem_pfn_range(i, nid, &start_pfn, &end_pfn, NULL) {
			...
			/* IAMROOT, 2024.08.08:
			 * - loop가 처음 순회할 때 zone_movable_pfn[nid]는 0이며
			 *   다음 loop에서 @nid가 동일하다면 다음중 하나의 pfn 값을 가짐.
			 *
			 *   1) (start_pfn && end_pfn) <= usable_startpfn 인 경우
			 *      DMA32 영역이며 이전 memory region의 end_pfn 값.
			 *   2) start_pfn + size_pages 값
			 *      kernelcore 저장을 위한 size를 계산중이므로
			 *      kernelcore_remaining가 0이 될때까지 zone_movable_pfn은
			 *      계속 업데이트 된다.
			 */
			...
			if (start_pfn < usable_startpfn) {
				...
				/* IAMROOT, 2021.12.04:
				 * - kernelcore에 사용할 page 개수를 계산하고 이를 바탕으로
				 *   remaining/required 변수 값을 보정한다.
				 */
				 ...
				 * - kernel core영역이 더 남아 있는 상황.
				 *   계속 진행해야되서 zone_movable_pfn이 갱신된다.
				 */
			 ...
			 */
			/* IAMROOT, 2024.08.10:
			 * - memblock memory region에서 start/end_pfn을 가져와 계산하고
			 *   이를 zone_movable_pfn[nid]에 설정하지만
			 *   kernelcore_remaining이 조금 남았다면 size_pages를 수정하여
			 *   그 이후는 zone movable 영역으로 사용한다.
			 */
			 ...
			 */
			/* IAMROOT, 2024.08.15:
			 * - size_pages를 기반으로 remaining/required 변수를 보정한다.
			 */
			required_kernelcore -= min(required_kernelcore,
								size_pages);
			kernelcore_remaining -= size_pages;
			/* IAMROOT, 2024.08.15:
			 * - kernelcore_remaining == 0 이라면 더 이상 kernelcore를 위해
			 *   phys-mem 영역을 할당할 필요가 없고 zone movable 용도로
			 *   사용해도 되니 loop를 빠져 나온다.
			 */
	 ...
	 */
	/* IAMROOT, 2021.12.04:
	 * - usable_nodes != 0 이면서 required_kernelcore가 남았다면 다른 node에
	 *   kernelcore 영역을 저장하기 위해 restart label에서 다시 수행한다.
	 */
	usable_nodes--;
...
/* IAMROOT, 2021.12.11:
 * - node(@pgdat)의 present_pages에 따라 상태를 업데이트하는 함수.
 */
static void check_for_memory(pg_data_t *pgdat, int nid)
{
	...
	/* IAMROOT, 2024.09.03:
	 * - node(@pgdat)의 zone을 순회하면서 해당 zone에 present_pages가
	 *   존재하면 그에 맞게 'node_stats'의 상태를 N_NORMAL_MEMORY로
	 *   업데이트한다.
	 */
...
void __init free_area_init(unsigned long *max_zone_pfn)
{
	...
	/* IAMROOT, 2024.08.15:
	 * - node 별로 ZONE DMA32, NORMAL, MOVABLE까지 계산 완료.
	 */
	 ...
	 */
	/* IAMROOT, 2021.12.04:
	 * - 모든 memblock.memory region을 순회하며 section마다
	 *   subsection_map (bitmap)을 초기화한다.
	 */
	...
	/* IAMROOT, 2021.12.11:
	 * - N_ONLINE 상태인 node를 순회하며 해당 node의 zone을 초기화하고
	 *   present pages가 존재하는 node에 한해 N_MEMORY flag를 set 한다.
	 */
	for_each_online_node(nid) {
		pg_data_t *pgdat = NODE_DATA(nid);
		free_area_init_node(nid);
		...
		/* IAMROOT, 2024.08.30:
		 * - ZONE_NORMAL을 가진 node는 N_NORMAL_MEMORY flag를 set 한다.
		 */
