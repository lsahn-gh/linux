 ...
 *
 * - AP(Application Processor, https://bsp-martin.tistory.com/12)
 *   cpu + peripheral  
 * - BP(Boot Processor)
 *
 * --- chat gpt ---
 * - 다중 프로세서 또는 코어가 있는 컴퓨터 시스템에서 "BP"라는 용어는 일반적으로 
 *   "부팅 프로세서"를 나타냅니다. 부트 프로세서는 시스템 전원을 켜거나 다시 
 *   시작할 때 초기화되는 첫 번째 프로세서입니다. 시스템의 하드웨어 구성 요소를 
 *   초기화하고 운영 체제 커널을 로드하는 BIOS 또는 UEFI 펌웨어 코드 실행을 
 *   담당합니다. 운영 체제 커널이 로드되면 부트 프로세서는 제어권을 커널로 넘긴 
 *   다음 시스템의 다른 프로세서를 "응용 프로그램 프로세서"(AP)로 초기화합니다.
 *
 *   "AP"는 "응용 프로그램 프로세서"를 나타냅니다. 애플리케이션 프로세서는 부트 
 *   프로세서가 아닌 멀티프로세서 시스템의 모든 프로세서입니다. 최신 컴퓨터 
 *   시스템에서 대부분의 프로세서는 일반적으로 애플리케이션 프로세서입니다. 
 *   이러한 프로세서는 시스템에서 응용 프로그램 및 서비스를 실행하는 코드 
 *   실행을 담당하며 운영 체제의 스케줄러에 의해 관리됩니다. 
 * ----------------
 */
/*
 * CPU-up			CPU-down
